...
import { localDateKey, addDaysKey, hmToMinutes } from './dates';
...
// Local calendar day. This used to be UTC, which flipped the day at 7pm.
const todayKey = () => localDateKey();

// How many finished days of food and sleep to keep. A year of summaries is
// well inside the browser's storage limit.
...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      // Finished days, newest last: the whole day's totals plus its entries,
      // with the goals that applied that day so history compares fairly.
      macroHistory: [],

      // Runs on app open, on return to the foreground, and every minute, so the
      // day turns over at midnight even when nothing gets logged.
      _rollIfNewDay: () => {
        const today = todayKey();
        const t = get().today;
        if (t.date > today) {
          // A day stamped in the future can only come from the old UTC key
          // (food logged after 7pm). It belongs to today, so keep it.
          set({ today: { ...t, date: today } });
          ...
          if (t.entries.length > 0) {
            ...
            const history = get().macroHistory.filter((d) => d.date !== t.date);
            history.push(archived);
            history.sort((a, b) => (a.date < b.date ? -1 : 1));
            set({ macroHistory: history.slice(-HISTORY_DAYS) });
          }
          set({ today: initialMacrosToday() });
        }
      ...
      // --- sleep ---
      // sleepLog[]: { date (the morning you woke), bedtime "HH:MM",
      //   wakeTime "HH:MM", hours, quality 1-5 }, oldest first
      sleepLog: [],

      logSleep: ({ date, bedtime, wakeTime, quality }) => {
        // A bedtime after the wake time means it was the previous evening.
        let mins = hmToMinutes(wakeTime) - hmToMinutes(bedtime);
        if (mins <= 0) mins += 1440;
        const hours = Math.round((mins / 60) * 100) / 100;
        const isNew = !get().sleepLog.some((n) => n.date === date);
        const list = get().sleepLog.filter((n) => n.date !== date);
        list.push({ date, bedtime, wakeTime, hours, quality });
        list.sort((a, b) => (a.date < b.date ? -1 : 1));
        set({ sleepLog: list.slice(-HISTORY_DAYS) });
        if (isNew) {
          get().addXp(10);
          get().checkInDaily();
        }
      ...
      deleteSleep: (date) => {
        set({ sleepLog: get().sleepLog.filter((n) => n.date !== date) });
      },
    ...
    }),
    {
      ...
      // Yesterday's totals must never be the first thing on screen.
      onRehydrateStorage: () => (state) => state?._rollIfNewDay(),
    }
  )
);
